...
describe('Auth Controller', () => {
  ...
  describe('register', () => {
    ...
    it('should return 400 if org is empty', async () => {
      req.body = { name: 'Test User', email: 'test@example.com', password: 'password123', org: {} };
      await authController.register(req, res, next);
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({ message: 'No such organisation exists' });
    });

    it('should call authService.registerUser and return response', async () => {
      req.body = { isAdmin: false, name: 'Test User', email: 'test@example.com', password: 'password123', org: { id: 'org123' } };
      const mockResponse = { status: 201, data: { message: 'User registered' } };
      authService.registerUser.mockResolvedValue(mockResponse);
      
      await authController.register(req, res, next);
      expect(authService.registerUser).toHaveBeenCalledWith(false, 'test@example.com', 'password123', 'Test User', { id: 'org123' });
